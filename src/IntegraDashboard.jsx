...
import {
  ...
  Layers3, Anchor, ChevronRight, ChevronDown, RotateCcw, Gauge, Upload, X,
...
function fillOneBarge(pool, bargeSize, target, allowedContractors) {
  ...
  while (filled < bargeSize - EPS && guard < 80) {
    ...
    const cands = pool.filter((p) => p.remaining > EPS && p.grade > 0 && allowedContractors.has(p.contractor));
    ...
    if (neutrals.length) {
      const n = neutrals[0]; const amt = Math.min(need, n.remaining);
      sources.push({ id: n.id, amt, grade: n.grade }); n.remaining -= amt; ni += amt * (n.grade / 100); filled += amt;
      continue;
    }
    ...
    if (highs.length && lows.length) {
      ...
      // reach the floor. If either side is supply-constrained, both amounts scale down
      // together so the ratio — and thus the exact-target result — is preserved.
      ...
      let scale = 1;
      if (idealAmtH > EPS && idealAmtH > h.remaining) scale = Math.min(scale, h.remaining / idealAmtH);
      if (idealAmtL > EPS && idealAmtL > l.remaining) scale = Math.min(scale, l.remaining / idealAmtL);
      const amtH = idealAmtH * scale, amtL = idealAmtL * scale;
      if (amtH > EPS) { sources.push({ id: h.id, amt: amtH, grade: h.grade }); h.remaining -= amtH; ni += amtH * (h.grade / 100); filled += amtH; }
      if (amtL > EPS) { sources.push({ id: l.id, amt: amtL, grade: l.grade }); l.remaining -= amtL; ni += amtL * (l.grade / 100); filled += amtL; }
    ...
    if (highs.length) {
      ...
      // dome as the banked ceiling headroom allows (accumulated from earlier
      // exact-target chunks), so on-spec-but-elevated ore isn't wasted.
      const h = highs[0];
      let amtH = Math.min(need, h.remaining);
      if (h.grade / 100 > maxT) {
      ...
      sources.push({ id: h.id, amt: amtH, grade: h.grade }); h.remaining -= amtH; ni += amtH * (h.grade / 100); filled += amtH;
      continue;
    }

    if (lows.length) {
      // No above-target material left — only dip into the banked floor surplus, never past it.
      const l = lows[0];
      const maxByFloor = Math.max(0, floorSurplus / (t - l.grade / 100));
      const amt = Math.min(need, l.remaining, maxByFloor);
      if (amt <= EPS) break; // no room left to dilute without breaching the floor — stop here
      sources.push({ id: l.id, amt, grade: l.grade }); l.remaining -= amt; ni += amt * (l.grade / 100); filled += amt;
      continue;
    }
          ...
          {!barge.finalized && (
            <>
              <div className="add-dome-row">
                <select value={addDome} onChange={(e) => setAddDome(e.target.value)}>
                  <option value="">+ Add dome…</option>
                  {availableDomes.map((p) => (
                    <option key={p.id} value={p.id}>{p.id} ({fmt(p.remaining)} WMT left, {p.grade}%)</option>
                  ))}
                </select>
                  ...
                  <tbody>
                    <tr>
                      <td className="inv-sig-box">
                        ...
                        <div className="inv-bold">DIREKTUR</div>
                      </td>
                    </tr>
                  </tbody>
                </table>
